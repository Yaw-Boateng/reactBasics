...
export default function EventCard({event}){
    return (
        <div className='bg-white rounded-lg shadow-lg p-6 flex flex-col items-center'>
            ...
            <img src={event.image} alt="Event Card" className='w-full rounded-md'/>
            <button className='bg-white text-purple-500 text-sm px-4 py-2 rounded-md absolute top-2 left-2 shadow-md'>FREE</button>
            </div>
            <h1 className='self-start'>{event.title}</h1>
            <h2 className='my-4 text-sub-paragraph text-purple-500 self-start'>{event.description}</h2>
